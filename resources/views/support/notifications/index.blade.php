<x-layout.app title="Notifications" pageTitle="Notifications">
    ...
    <div class="page-header">
        <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:12px;">
            ...
            @if(auth()->user()->readNotifications()->count() > 0)
                <form method="POST" action="{{ route('support.notifications.deleteRead') }}">
                    @csrf
                    @method('DELETE')
    ...
    @if(auth()->user()->unreadNotifications->count() > 0)
        <div style="margin-bottom:16px;">
            <form method="POST" action="{{ route('support.notifications.markAllRead') }}">
                @csrf
                @method('PATCH')
    ...
    <div class="card">
        ...
        <div style="display:flex; align-items:flex-start; gap:14px; padding:16px 20px;
            ...
            <div style="display:flex; gap:6px; flex-shrink:0;">
                @if($isUnread)
                    <form method="POST" action="{{ route('support.notifications.read', $notif->id) }}">
                        @csrf @method('PATCH')
