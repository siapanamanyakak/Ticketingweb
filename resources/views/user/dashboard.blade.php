...
<div style="display:grid; grid-template-columns:1fr 280px; gap:16px; margin-bottom:16px;">
    ...
    {{-- Welcome Card --}}
    <div style="background:linear-gradient(135deg, var(--navy-800) 0%, var(--navy-600) 100%);
...
<div style="display:grid; grid-template-columns:repeat(5, 1fr); gap:12px; margin-bottom:16px;">
    ...
    @foreach($statItems as $stat)
        ...
        <div style="background:white; border-radius:12px; padding:16px;
                    box-shadow:0 1px 4px rgba(0,0,0,0.06);">
            <div style="width:36px; height:36px; background:{{ $c['bg'] }}; border-radius:8px;
                        display:flex; align-items:center; justify-content:center; margin-bottom:10px;">
                <svg fill="none" stroke="{{ $c['color'] }}" viewBox="0 0 24 24" width="18" height="18">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="{{ $stat['icon'] }}"/>
                </svg>
            </div>
...
<style>
    @media (max-width: 768px) {
        ...
        .bento-stats { grid-template-columns: repeat(3, 1fr) !important; }
    }
    @media (max-width: 480px) {
        .bento-stats { grid-template-columns: repeat(2, 1fr) !important; }
    }
</style>
@endpush
